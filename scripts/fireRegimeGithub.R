...
d2c.intp <- approx(x=d2c$age, y=d2c$d2c, xout=agest, ties="mean")
toc.intp <- approx(x=toc$age, y=toc$TOC, xout=agest, ties="mean")
d2ctoc.dat <- data.frame(d2c.intp,toc.intp$y)
...
## only 'wet' periods (> 5.1% total organic carbon; lowest value from 11 ka to the present)
...
head(comb.dat)
combwet <- subset(comb.dat, toc >= 5.41750589)  
...
iter <- 10000
tsplit.thresh.vec <- seq(65000,1000,-1000)
